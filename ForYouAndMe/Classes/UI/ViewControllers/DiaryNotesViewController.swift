        ...
        if let typeNote = diaryNote.diaryNoteType {
            ...
            case .eaten:
                ...
                cell.display(data: diaryNote, onTap: { [weak self] in
                    guard let self = self else { return }
                    self.navigator.openEatenFormViewController(presenter: self, foodItem: diaryNote)
                })
            ...
            case .doses:
                ...
                cell.display(data: diaryNote, onTap: { [weak self] in
                    guard let self = self else { return }
                    self.navigator.openDosesFormViewController(presenter: self, dosesItem: diaryNote)
                })
